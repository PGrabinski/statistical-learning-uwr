---
title: "Report 1: Vector and Matrix Algebra, Multivariate Normal Distribution"
...
As noted during the lecture, the eigenvectors scaled by the square root of the eigenvalues give the directions of the axis of the ellipses. The ellipses are the constant density contours of the joint distribution. The vectors are orthogonal, but due to the radically different orders of magnitude of the eigenvalues and scales on the plot we can't see that.
...
The best guess for the length of the child is the mean of the distribution as the probability density has the highest value there, i.e. $49.8\; cm$.
...
First, let's construct a new $X$ vector that is $3p$-dimensional and is a concatenation of $X_1$, $X_2$ and $X_3$:
...
And $I_{p\times p}$ is $p\times p-$dimensional identity matrix. It's dimension is $p\times 3p$.
...
The transformed data seems to be decorellated - the constant-density ellipsoids would be much more round than the original ones. The data is transformed into the eigenvector space, where the covariance matrix is diagonal.
...
The quantile plot for the length marginal distribution:

![Parents length qq plot](parents_length_qqplot.png)

The quantile plot for the weight marginal distribution:

![Parents weight qq plot](parents_weight_qqplot.png)

The quantile plot for the father height marginal distribution:

![Parents father height qq plot](father_height_qqplot.png)

The quantile plot for the mother height marginal distribution:
